...
namespace Modules\MetaAble;

use Illuminate\Database\Eloquent\Builder;
use Illuminate\Database\Eloquent\Collection;
use Illuminate\Database\Eloquent\Relations\MorphMany;
use Illuminate\Database\Query\JoinClause;
use Modules\MetaAble\Entities\MetaAble;
use Traversable;

/**
 * @mixin \Eloquent
 *
 * @method self WhereHasMeta($key)
 * @method self WhereHasMetaKeys($keys)
 * @method self OrderByMeta(string $key, string $direction = 'asc', $strict = false, $field = 'value')
 */
trait MetaAbleTrait
{
    /**
     ...
     */
    public static function bootMetaAbleTrait()
    {
    ...
    {
        return $this->morphMany($this->getMetaClassName(), 'able');
    }
    ...
    {
        return $this->getMetaCollection()->toBase()->map(function ($meta) {
     ...
     * @param string  $field
     */
    public function scopeOrderByMeta(Builder $q, string $key, string $direction = 'asc', $strict = false, $field = 'value')
    {
     ...
     * @param string  $field
     */
    public function scopeOrderByMetaNumeric(Builder $q, string $key, string $direction = 'asc', $strict = false, $field = 'value')
    {
        $table = $this->joinMetaTable($q, $key, $strict ? 'inner' : 'left');
        $direction = 'asc' == strtolower($direction) ? 'asc' : 'desc';
        $field = $q->getQuery()->getGrammar()->wrap("{$table}.{$field}");
    ...
    private function joinMetaTable(Builder $q, string $key, $type = 'left')
    {
        ...
        $alias = str_replace('.', '_', $metaTable).'__'.$key;
        ...
        $q->join("{$metaTable} as {$alias}", function (JoinClause $q) use ($relation, $key, $alias) {
            ...
            $q->on($relation->getQualifiedParentKeyName(), '=', $alias.'.'.$foreign_key)
                ->where($alias.'.key', '=', $key)
                ->where($alias.'.'.$type, '=', $this->getMorphClass());
        }, null, null, $type);
     ...
     * @return MetaAble
     */
    protected function makeMeta(string $key = '', $value = '')
    {
